...
internal class ByMethods
{
    ...
    private static void IntersectBy(IEnumerable<Cyclist> evenAgedPeople, IEnumerable<Cyclist> personAbove30)
    {
        ...
        IEnumerable<Cyclist> intersectionBy = evenAgedPeople.IntersectBy(personAbove30.Select(x=>x.Age), cyclist => cyclist.Age);
        //Roman
        Console.WriteLine($"IntersectionBy: {string.Join(",", intersectionBy.Select(person => person.Name))}");
    }
    ...
    private static void DistinctBy(IEnumerable<Cyclist> evenAgedPeople, IEnumerable<Cyclist> personAbove30)
    {
        ...
        IEnumerable<Cyclist> distinctBy = evenAgedPeople.DistinctBy(x => x.Age);
        //Roman, Roma
        Console.WriteLine($"DistinctBy: {string.Join(",", distinctBy.Select(person => person.Name))}");
    }
    ...
    private static void ExceptBy(IEnumerable<Cyclist> evenAgedPeople, IEnumerable<Cyclist> personAbove30)
    {
        ...
        var exceptBy = evenAgedPeople.ExceptBy(personAbove30.Select(x=>x.Age), c=>c.Age);
        //Roma
